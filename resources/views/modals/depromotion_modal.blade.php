<div class="fixed z-50 inset-0 overflow-y-auto bg-gray-900 bg-opacity-20 flex justify-center items-center text-left " wire:ignore.self>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-96 p-4  h-[83vh] overflow-y-auto">
        ...
        <form wire:submit.prevent="{{$submit_form}}">
            ...
            <div class="mb-4">
                <label for="from_date" class="block mb-1 text-gray-600 dark:text-green-500 font-arial">ရာထူးလျော့ သည့်နေ့(မှ)</label>
                <input required type="date" wire:model="from_date" id="from_date" class="font-arial bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5">
                @error('from_date') <span class="mt-1 text-red-500 text-xs font-arial font-semibold">{{ $message }}</span> @enderror
            </div>
            <div class="mb-4">
                <label for="to_date" class="block mb-1 text-gray-600 dark:text-green-500 font-arial">ရာထူးလျော့ သည့်နေ့(အထိ)</label>
                <input required type="date" wire:model="to_date" id="to_date" class="font-arial bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5">
            ...
            <div class="mb-4">
                <label for="depromoted_rank_name" class="block mb-1 text-gray-600 dark:text-green-500 font-arial">
                    ရာထူးတိုးလျော့
                </label>
                <div class="relative">
                    <select
                        wire:model="depromoted_rank_name"
                        id="depromoted_rank_name"
                        class="text-sm font-arial block w-full mb-4 p-2.5 border border-gray-300 rounded-md shadow-sm focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50">
                        <option value="" selected>ရာထူးတိုးလျော့</option>
                        @foreach ($ranks as $rank)
                            <option value="{{ $rank->id }}">{{ $rank->name }}</option>
                        @endforeach
                    </select>
                    @error('depromoted_rank_name')
                    <span class="mt-1 text-red-500 text-xs font-arial font-semibold">{{ $message }}</span>
                    @enderror
                </div>
            </div>
