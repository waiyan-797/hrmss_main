        ...
        <tbody class="">
            @if($column_vals)
                @foreach($column_vals as $index => $column_val)
                    <tr class="border-b font-arial dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-600 
                              {{$index % 2 ? 'bg-white hover:bg-gray-50' : 'bg-gray-200 hover:bg-white'}}">
                        
                        @if ($column_vals === $educations)
                            <td class="px-4 py-4 min-w-[200px] text-gray-500 dark:text-gray-300">{{$column_val['education']}}</td>
                            <td class="px-4 py-4 min-w-[200px] text-gray-500 dark:text-gray-300">{{$column_val['education_type']}}</td>
                            <td class="px-4 py-4 min-w-[200px] text-gray-500 dark:text-gray-300">{{$column_val['education_group']}}</td>
                            <td class="px-4 py-4 min-w-[200px] text-gray-500 dark:text-gray-300">{{$column_val['country_id']}}</td>
                            <td class="px-4 py-4 min-w-[200px] text-gray-500 dark:text-gray-300">
                                <a href="{{ asset('storage/upload/'.$column_val['degree_certificate']) }}" target="_blank" class="text-blue-600 hover:text-blue-800">
                                    <i class="fa-solid fa-file size-[2rem] text-gray-500 dark:text-gray-300"></i>
                                </a>
                            </td>
