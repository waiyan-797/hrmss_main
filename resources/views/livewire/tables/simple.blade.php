<div class="w-full overflow-x-auto">
    <table class="w-full text-sm rounded-lg table-auto">
        ...
        <tbody class="">
            {{-- @dd($column_vals) --}}
            @foreach ($column_vals as $index => $name)

            {{-- @dd($name['id']) --}}
            @if($action_name === 'recommendations')
            <tr
                ...
                <td class="px-4 py-4 min-w-[200px] text-gray-500 dark:text-gray-300">{{$name['recommend_by']}}</td>
                <td class="px-4 py-2 min-w-[80px]">

                    <button type="button"
                        wire:click='edit_recommendation_modal("recommendations_modal",{{$index}})'
                    ...
                    {{-- <button type="button" wire:click="{{$del_method}}({{$index}},{{$name['id']}})"
                    ...
                    <button type="button" wire:click="confirmRemoveRecommendation({{$index}}, {{$name['id']}})"
...
<script type='defer'>
    ...
    Livewire.on('alert', ({ type, message }) => {
        Swal.fire({
            icon: type,
            title: type === 'success' ? 'အောင်မြင်ပါပြီ' : 'မှားယွင်းမှု',
            text: message,
            timer: 2000,
