<div class="w-full">
    <div class="flex justify-center w-full h-[83vh] overflow-y-auto">
        <div class="w-full mx-auto px-3 py-4">
            ...
            <h2 class="text-center text-base font-bold mb-1">အသက် ၁၈ နှစ်နှင့်အထက်
            ...
            <table class="md:w-full">
                ...
                <tbody>
                    ...
                    <tr class="font-bold">
                        ...
                        <td class="border border-black text-center p-2">{{ en2mm($second_ranks->sum(fn($rank) => $rank->staffs->where('gender_id', 1)->filter(fn($staff) => Carbon\Carbon::parse($staff->dob)->diffInYears(Carbon\Carbon::now()) > 18)->count())) }}</td>
                        <td class="border border-black text-center p-2">{{ en2mm($second_ranks->sum(fn($rank) => $rank->staffs->where('gender_id', 2)->filter(fn($staff) => Carbon\Carbon::parse($staff->dob)->diffInYears(Carbon\Carbon::now()) > 18)->count())) }}</td>
                        <td class="border border-black text-center p-2">{{ en2mm($second_ranks->sum(fn($rank) => $rank->staffs->filter(fn($staff) => Carbon\Carbon::parse($staff->dob)->diffInYears(Carbon\Carbon::now()) > 18)->count())) }}</td>
                    </tr>
                    <tr>
                        <td class="border border-black text-center p-2">၁</td>
                        <td class="border border-black text-center p-2">နေ့စား</td>
                        <td class="border border-black text-center p-2">{{ en2mm($third_ranks->sum(fn($rank) => $rank->staffs->where('gender_id', 1)->filter(fn($staff) => Carbon\Carbon::parse($staff->dob)->diffInYears(Carbon\Carbon::now()) > 18)->count())) }}</td>
                        <td class="border border-black text-center p-2">{{ en2mm($third_ranks->sum(fn($rank) => $rank->staffs->where('gender_id', 2)->filter(fn($staff) => Carbon\Carbon::parse($staff->dob)->diffInYears(Carbon\Carbon::now()) > 18)->count())) }}</td>
                        <td class="border border-black text-center p-2">{{ en2mm($third_ranks->sum(fn($rank) => $rank->staffs->filter(fn($staff) => Carbon\Carbon::parse($staff->dob)->diffInYears(Carbon\Carbon::now()) > 18)->count())) }}</td>
                    </tr>
                    <tr class="font-bold">
                        <td class="border border-black text-center p-2"></td>
                        <td class="border border-black text-center p-2">စုစုပေါင်း ဝန်ထမ်းဦးရေ</td>
                        <td class="border border-black text-center p-2">{{ en2mm($all_ranks->sum(fn($rank) => $rank->staffs->where('gender_id', 1)->filter(fn($staff) => Carbon\Carbon::parse($staff->dob)->diffInYears(Carbon\Carbon::now()) > 18)->count())) }}</td>
                        <td class="border border-black text-center p-2">{{ en2mm($all_ranks->sum(fn($rank) => $rank->staffs->where('gender_id', 2)->filter(fn($staff) => Carbon\Carbon::parse($staff->dob)->diffInYears(Carbon\Carbon::now()) > 18)->count())) }}</td>
                        <td class="border border-black text-center p-2">{{ en2mm($all_ranks->sum(fn($rank) => $rank->staffs->filter(fn($staff) => Carbon\Carbon::parse($staff->dob)->diffInYears(Carbon\Carbon::now()) > 18)->count())) }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
