...
<div x-data="{
    ...
    dropdownStyles: '',
    get filteredOptions() {
        let filtered = this.search.trim()
            ? this.options.filter(option => option.name.toLowerCase().includes(this.search.toLowerCase()))
            : this.options;

        return this.selectedId ? filtered.slice(0, 10) : filtered; // Limit to 10 in edit mode
    },
    get selectedOption() {
        if (!this.selectedId) return null;
        return this.options.find(option => option.id === this.selectedId);
