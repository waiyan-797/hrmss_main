...
class StaffList3 extends Component
{
    ...
    { 
        ...
        $section = $phpWord->addSection([
            ...
            'marginRight' => \PhpOffice\PhpWord\Shared\Converter::inchToTwip(0.51),   
            'marginTop'   => \PhpOffice\PhpWord\Shared\Converter::inchToTwip(0.23),  
    ...
    $all_ranks = Rank::where('is_dica', 1)
        ...
        foreach ($first_ranks as $index => $rank) {
            ...
            $table->addCell(2000)->addText(en2mm($rank->staffs->where('sgender_id', 1)->count() + $rank->staffs->where('gender_id', 2)->count()),null,['alignment'=>'center','spaceBefore'=>50, 'indentation' => ['left' => 100],'lineHeight'=>0.6 ]);
        }
        $table->addRow();
