...
->when($this->education_id, fn($query) =>
    ...
    $q->with(['education' => function ($query) {
        ...
        $table->addCell(3000)->addText('ပညာအရည်အချင်း', ['bold' => true]);
        foreach ($staffs as $index=> $staff) {
            $table->addRow();
            $table->addCell(700)->addText(en2mm((int) $index + 1));
            $table->addCell(2200)->addText($staff->name);
            $table->addCell(3500)->addText($staff->current_rank?->name);
            $educationNames = $staff->staff_educations->map(fn($edu) => $edu->education?->name)->implode(', ');
            $table->addCell(3000)->addText($educationNames);
        }
        $fileName = 'E07.docx';
