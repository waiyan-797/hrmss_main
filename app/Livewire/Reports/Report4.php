...
class Report4 extends Component
{
    ...
    //                 $table->addCell(3000)->addText($staff->abroads[$i]->countries->pluck('name')->unique()->join(', '),null,$pStyle_2);
    ...
    public function go_word()
    {
        ...
        $pStyle = ['align' => 'center', 'spaceAfter' => 200];
        $table->addRow();
        $table->addCell(1000)->addText('စဥ်', ['bold' => true], $pStyle);
        $table->addCell(3000)->addText('အမည်', ['bold' => true], $pStyle);
        $table->addCell(3000)->addText('ရာထူး', ['bold' => true], $pStyle);
        $table->addCell(2000)->addText('သွားရောက်သည့်ကာလ (မှ)', ['bold' => true], $pStyle);
        $table->addCell(2000)->addText('ထိ', ['bold' => true], $pStyle);
        $table->addCell(3000)->addText('သွားရောက်သည့်နိုင်ငံ', ['bold' => true], $pStyle);
        $table->addCell(4000)->addText('ပြည်ပသို့သွားရောက်ခဲ့သောအကြောင်းအရာ', ['bold' => true], $pStyle);
        $table->addCell(3000)->addText('ထောက်ပံ့သည့်အဖွဲ့အစည်း', ['bold' => true], $pStyle);
        foreach ($staffs as $index => $staff) {
            $maxRows = max($staff->trainings->count(), $staff->abroads->count());
            ...
            for ($i = 0; $i < $maxRows; $i++) {
                $table->addRow();
                if ($i == 0) {
                    $table->addCell(1000, ['vMerge' => 'restart'])->addText(en2mm($index + 1), null, $pStyle);
                    $table->addCell(3000, ['vMerge' => 'restart'])->addText($staff->name, null, $pStyle);
                    $table->addCell(3000, ['vMerge' => 'restart'])->addText($staff->currentRank->name ?? '', null, $pStyle);
                } else {
                    $table->addCell(1000, ['vMerge' => 'continue']);
                    $table->addCell(3000, ['vMerge' => 'continue']);
                    $table->addCell(3000, ['vMerge' => 'continue']);
                ...
                if (isset($staff->abroads[$i])) {
                    $abroad = $staff->abroads[$i];
                    $table->addCell(2000)->addText(formatDMYmm($abroad->from_date), null, $pStyle);
                    $table->addCell(2000)->addText(formatDMYmm($abroad->to_date), null, $pStyle);
                    $table->addCell(3000)->addText($staff->abroads[$i]->countries->pluck('name')->unique()->join(', '), null, $pStyle);
                    $table->addCell(4000)->addText($abroad->particular, null, $pStyle);
                    $table->addCell(3000)->addText($abroad->sponser, null, $pStyle);
                } else {
                    $table->addCell(2000);
                    $table->addCell(2000);
                    $table->addCell(3000);
