...
class Increment extends Component
{
    ...
    {
        $this->resetValidation();
        $this->reset([ 'staff_name', 'increment_rank_name', 'min_salary', 'increment', 'max_salary', 'increment_date', 'increment_time', 'order_no', 'remark' ]);
    ...
    public function submitForm()
    {
        $this->confirm_add ? $this->createincrement() : $this->updateincrement();
    }
    ...
    public function render()
    {
        ...
        $incrementQuery = ModelsIncrement::where('staff_id', $this->staff_id)->with(['staff', 'rank']);

        if ($this->increment_search) {
            $this->resetPage();
            $searchTerm = '%' . $this->increment_search . '%';
            $incrementQuery->where('order_no', 'LIKE', $searchTerm)->orWhereHas('staff', function ($query) use ($searchTerm) {
                $query->where('name', 'LIKE', $searchTerm);
            });
        }

        $increments = $incrementQuery->paginate(10);
